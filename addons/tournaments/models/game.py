...
class Game(models.Model):
    ...
    _sql_constraints = [
        ("name_uniq", "UNIQUE(name)", "A Game with the same name already exists. Names must be uniques"),
        ("tag_uniq", "UNIQUE(tag)", "A Game with the same tag already exists. Tags must be uniques")
    ]
    ...
    pegi_age_logo = fields.Image(
        string="Game PEGI Age Label",
        related="pegi_age_id.logo"
    )
