...
from services import pane as pane_service
...
class ShowcaseWorker:
    ...
    def __init__(self, odoo_env: Environment, websocket: WebSocket, showcase_id: int) -> None:
        super().__init__()
            ...
            try:
                ...
                with registry.manage_changes():
                    with registry.cursor() as cr:
                        ...
                        pane_id: int = pane_service.get_next_showcase_pane_id(odoo_env, self._showcase_id)

                        pane_duration: int
                        pane_data: dict
                        pane_duration, pane_data = pane_service.get_pane_dict(odoo_env, pane_id)

                        duration = float(pane_duration) / 1000

                        command: dict = showcase_service.prepare_command(CommandType.PANE, pane_data)
