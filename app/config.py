...
class ConfigSkeleton:
    ...
    def config_print(self, logging_method: Callable = print, hide_secrets: List[str] = None) -> None:
...
class Config(ConfigSkeleton):
    ...
    LOG_FORMAT: str = "%(asctime)s,%(msecs)d [%(thread)s|%(process)s] {%(processName)s} [%(levelname)s] %(name)s %(funcName)s {%(pathname)s:%(lineno)d}: %(message)s"
    LOG_ACCESS_FORMAT: str = "%(asctime)s - %(client_addr)s - \"%(request_line)s\" %(status_code)s"

    DB_HOST: str = "postgres"
    DB_PORT: int = 5432
    DB_USERNAME: str = "giocomix"
    DB_PASSWORD: str = "giocomix"
    DB_NAME: str = "giocomix"

    ADDONS_PATH: list[str] = ["../odoo/odoo/addons", "../odoo/addons", "../addons", "../web"]
