...
def get_next_showcase_pane_id(odoo_env: Environment, showcase_id: int = 0) -> int:
...
def get_pane_dict(odoo_env: Environment, pane_id: int) -> tuple[int, dict]:
    ...
    if showcase_pane.pane_id.type_id == odoo_env.ref("showcases.data_tournaments_pane_type_text"):
        pane_data["params"] = {
            ...
            "backgroundImage": generate_image(showcase_pane.pane_id.text_background_file),
        }

    elif showcase_pane.pane_id.type_id == odoo_env.ref("showcases.data_tournaments_pane_type_image"):
        pane_data["params"] = {
            "mimeType": showcase_pane.pane_id.image_file_mimetype,
            "imageData": showcase_pane.pane_id.image_file.decode()
        }

    elif showcase_pane.pane_id.type_id == odoo_env.ref("showcases.data_tournaments_pane_type_video"):
        pane_data["params"] = {
            "videoUrl": showcase_pane.pane_id.video_url
        }

    elif showcase_pane.pane_id.type_id == odoo_env.ref("showcases.data_tournaments_pane_type_tournament_entrants_list"):
        ...
        col_view: int = showcase_pane.pane_id.tournament_classification_column_per_view or 4
        item_col: int = showcase_pane.pane_id.tournament_classification_items_per_column or 18

        show_scheduled_time: bool = showcase_pane.pane_id.tournament_matches_show_scheduled_time
        ...
        pane_data["params"] = {
            ...
            "entrants": [generate_entrant_tournament_availability(entrant) for entrant in entrants],
        ...
        col_view: int = showcase_pane.pane_id.tournament_classification_column_per_view or 3
        item_col: int = showcase_pane.pane_id.tournament_classification_items_per_column or 15
        ...
        for entrant in entrants:
            ...
            params_entrants.append({
                ...
                "type": entrant.type,
                "results": [getattr(match_result, field_name) for field_name in classification_config["field_names"]],
                "sort": [getattr(match_result, field_name) for field_name in classification_config["sort_field_names"]],
                "position": 0  # TODO: Compute position
            })

        params_entrants = sorted(
            params_entrants,
            key=lambda x: tuple(x["sort"][pos] * direction for pos, direction in classification_config["sort"]),
            reverse=True
        )
        ...
        pane_data["params"] = params
            ...
            "showcases.data_tournaments_pane_type_tournament_best_of_matches") or showcase_pane.pane_id.type_id == odoo_env.ref(
        ...
        params_matches: list[dict] = generate_matches(matches, result_config, show_incomplete_match=False)

        params["matches"] = params_matches
        pane_data["params"] = params

    elif showcase_pane.pane_id.type_id == odoo_env.ref("showcases.data_tournaments_pane_type_tournament_brackets"):
        match_obj = odoo_env["tournaments.match"]
...
def generate_matches(matches, result_config, show_incomplete_match=True) -> list[dict]:
    ...
    for match in matches:
        ...
        for match_entrant in match.match_entrant_ids:
            entrants[match_entrant.order_num] = generate_entrant(match_entrant.entrant_id)
        ...
        for match_entrant in match.match_entrant_ids:
            ...
            field_names: list[str] = result_fields[match.match_entrant_ids.ids.index(match_entrant.id)]
            for match_result in match.match_result_ids:
                field_values: list = [getattr(match_result, field_name) for field_name in field_names]
